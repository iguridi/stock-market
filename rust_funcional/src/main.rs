...
mod chart {
    ...
    pub type StringGrid = Vec<Vec<String>>;

    pub fn show(max: i32, history: &History) {
        let mut chart: StringGrid = (0..max + 1)
            .map(|_| (0..TIME).map(|_| "  ".to_owned()).collect())
            .collect();
        for turn in 0..TIME as usize {
            let (from, to) = (history.min[turn] as usize, history.max[turn] as usize);
            for price in from..to {
                chart = put(chart.clone(), price, turn, " |".to_owned());
            }
            chart = put(chart.clone(), from, turn, " +".to_owned());
            chart = put(chart.clone(), to, turn, " +".to_owned());
        }
        ...
        print!(" |_{}__ time({})", underscores, TIME);
    }

    fn put(grid: StringGrid, price: usize, turn: usize, icon: String) -> StringGrid {
        let len = grid.len();
        let mut grid = grid.clone();
        grid[len - price][turn] = icon;
        grid
    }
}
